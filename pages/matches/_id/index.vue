    ...
    <v-row v-if="!loading">
      ...
      <v-col cols="12" sm="4" md="4" lg="4">
        <v-card>
          ...
          <v-card-title class="subheading font-weight-bold" v-else>Queuing</v-card-title>
          ...
          <v-card-subtitle class="text-left" v-else-if="queueInfo.event == 'joinServer'">
            Server ready <span class="float-right">{{ queueInfo.map }}</span>
          </v-card-subtitle>
          ...
          <v-card-text v-else-if="queueInfo.event == 'joinServer' && queueInfo.ip">
            <v-row>
              <v-col cols="12">
                <v-btn color="red" raised :href="'steam://connect/'+queueInfo.ip+'/'+queueInfo.password">Join Server</v-btn>
              </v-col>
              <v-col cols="12">
                <v-text-field class="caption" :append-icon="showIP ? 'mdi-eye' : 'mdi-eye-off'" :type="showIP ? 'text' : 'password'" @click:append="showIP = !showIP" readonly outlined dense filled :value="queueInfo.connect"></v-text-field>
              </v-col>
            </v-row>
          </v-card-text>
          <v-card-text v-else-if="queueInfo.event == 'settingServer'">
            <v-progress-circular size="70" width="7" :color="queueInfo.event == 'joinServer' ? 'primary' : 'red'" indeterminate></v-progress-circular>
          </v-card-text>

          <v-card-actions v-if="!queueInfo.event">
